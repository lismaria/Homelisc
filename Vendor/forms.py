...
class ItemImageUploadForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('item_img',)
        widgets = {
            'item_img': forms.ClearableFileInput(attrs={'multiple': True, 'type':'file' ,'id':'add-itemimg','style':'display:none', 'accept':'image/*', 'class':"change-pic{{i}}", 'onchange':"document.getElementById('item-pic{{i}}').src = window.URL.createObjectURL(this.files[0])"})
        }
        # required= {
        #     'item_img': False
        # }
